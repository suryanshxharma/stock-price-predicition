...
st.write("This interactive application runs and compares sequential deep learning models (LSTM, GRU) against traditional ML baselines (Linear Regression, Random Forest) to forecast stock close prices.[...]
...
if has_artifacts:
    ...
    with tab_overview:
        ...
        st.info("**Key Insight for Interviews:** Traditional baselines like Linear Regression and Random Forest serve as standard validation. If the deep learning sequential models (LSTM, GRU) do not [...]
    ...
    with tab_indicators:
        ...
        st.write("We expanded the stock features beyond closing price to engineer **21 features** capturing trend, momentum, volatility, and volume signals. This provides deep feature representations [...]
        ...
        st.markdown("""
        * **Trend Indicators**: Simple Moving Averages (`SMA_5`, `SMA_10`, `SMA_20`) and Exponential Moving Averages (`EMA_12`, `EMA_26`). Bollinger Bands (`BOL_UPPER`, `BOL_LOWER`) capture volatility[...]
    ...
    with tab_guide:
        ...
        with st.expander("Q1: Why can we NOT randomly split time-series data into train, validation, and test sets?"):
            ...
            Random shuffling (standard `train_test_split`) causes <strong>temporal data leakage</strong>. In time series, data points are highly correlated across time. If you randomly shuffle, inform[...]
            <br><br>
            <strong>Our Solution:</strong> We implement a strict <strong>chronological split</strong> (70% oldest data for training, 15% middle for validation, 15% newest for test) to evaluate the mod[...]
            """, unsafe_allow_html=True)
        ...
        with st.expander("Q3: What are the differences between LSTM and GRU architectures?"):
            ...
            <ul>
                ...
                <li><strong>GRU (Gated Recurrent Unit):</strong> Merges the cell state and hidden state, and features only two gates (reset and update). It has fewer parameters, making it computationa[...]
            </ul>
        ...
        with st.expander("Q4: Why does a model sometimes forecast a 'shifted' version of the actual price?"):
            ...
            <br><br>
            <strong>How to inspect this:</strong> Residual plots and **Directional Accuracy** (predicting the sign of return correctly) help diagnose this. If Directional Accuracy is around 50%, the m[...]
            """, unsafe_allow_html=True)
